...
// TRIP_TIMES_STORAGE_KEY, googleRoutesApiKey, requestGpsPosition,
// getDriveTimeMinutes, timeToMinutes, minutesToClock, minutesToDuration, and
// computeSchedule all come from charts.js (loaded before this file) — shared
// with the Week Ahead page, which offers the same trip-schedule calculation.
...
// fmtNaive, hourOf, dateOnly, naiveMsToLocalDate, computeWindowsForLocation,
// average, DAY_COLORS, conditionColor, and the location/type filter chip
// functions all come from charts.js (loaded before this file) — shared with
// the Week Ahead page, which needs the exact same session-computation logic.
...
async function init() {
  ...
  renderLocationChips(allLocations, selectedLocations, render);
  renderTypeChips(selectedTypes, render);
