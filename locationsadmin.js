...
async function init() {
  ...
  // own comment on why. All start folded — Groups/Mark Lists are
 ...
 * what every section below should be showing. Location Groups and Fishing
 * Mark Lists are available to ANY signed-in user (locations themselves and
 * homes are managed on the Map); Refresh data now, Users, and Tiers are all
 ...
 *
 * locationGroups is a flat array of plain name strings (a location's own groups are ticked in the Map's location
 * editor, js/location-editor.js). groupNameToId holds each name's real D1 id so this section's own add/remove calls
 * know which row to hit.
 */
