...
function mountLocationPill(frameId, { nameId, subId, tileIds = [] }) {
  ...
  // Optional gear right after the name (the Map page's location editor — see setGear below); hidden until a page
  // hands it something to do.
  const gearBtn = document.createElement("button");
  gearBtn.type = "button";
  gearBtn.className = "loc-pill-gear-btn";
  gearBtn.hidden = true;
  gearBtn.setAttribute("aria-label", "Edit this location");
  gearBtn.title = "Edit this location";
  gearBtn.innerHTML = '<svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" aria-hidden="true"><path d="M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58a.49.49 0 0 0 .12-.61l-1.92-3.32a.49.49 0 0 0-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54a.48.48 0 0 0-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96a.49.49 0 0 0-.59.22L2.74 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58a.49.49 0 0 0-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.6A3.6 3.6 0 1 1 12 8.4a3.6 3.6 0 0 1 0 7.2z"/></svg>';
  let gearHandler = null;
  gearBtn.addEventListener("click", () => {
    if (gearHandler) gearHandler();
  });
  ...
  return {
    /** Shows the gear after the name and runs `handler` on a click; null hides it again. */
    setGear(handler) {
      gearHandler = handler || null;
      gearBtn.hidden = !gearHandler;
    },
    setPhoto(type) {
